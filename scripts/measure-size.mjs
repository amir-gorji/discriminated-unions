 ...
 *      number that the cap of 2.5 KB applies to.
...
console.log(
  `\nDismatch bundle size — cap ${(CAP_BYTES / 1024).toFixed(1)} KB\n`,
);
console.log(`Canonical (esbuild --bundle --minify, non-gzipped):`);
console.log(
  `  ${canonicalStatus} src/index.ts → ${canonicalBytes} B (${canonicalKb} KB)\n`,
);
...
const publishedTargets = [
  {
    label: 'lib/index.mjs (ESM, ws preserved)',
    file: path.join(rootDir, 'lib/index.mjs'),
  },
  {
    label: 'lib/index.js  (CJS, fully minified)',
...
if (canonicalBytes > CAP_BYTES) {
  console.error(
    `Canonical bundle ${canonicalBytes} B exceeds cap ${CAP_BYTES} B.`,
  );
  process.exit(1);
}
